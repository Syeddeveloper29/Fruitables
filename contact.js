...
function handleMouseOver() {
  if (window.innerWidth <= 1045) {
    ...
    contactLink.style.transition = `transform 0.4s ease-in-out`
  };
};
...
// Dark & Light Mode Toggle Button starts here
...
const successful = document.querySelector('.successful-notification');
const invalid = document.querySelector('.invalid-notification');
const successBtn = document.querySelector('.successful-button');
...
Contactform.addEventListener('submit', (e) => {
  ...
  const fullName = namee.value.trim();
  const contactEmail = email.value.trim();
  const phoneNumber = number.value.trim();
  const homeAddress = address.value.trim();


  // get existing users from local storage
  let storedUsers = JSON.parse(localStorage.getItem('contactUsers')) || [];
  ...
  const existingUser = storedUsers.some(user => user.email === contactEmail);

  // show appropriate message
  if (existingUser) {
    invalid.style.visibility = `visible`;
  } else {
    storedUsers.push(newContactUser);
    localStorage.setItem('contactUsers', JSON.stringify(storedUsers));
    successful.style.visibility = `visible`;
  };

  // button handlers 
  successBtn.addEventListener('click', () => {
    successful.style.visibility = 'hidden';
    form.reset();
  });
